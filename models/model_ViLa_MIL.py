...
def load_conch_to_cpu(checkpoint_path="/home/txiang/pathology/CLAM/models/conch_checkpoint.bin"):
    model_cfg = 'conch_ViT-B-16'
    model, preprocess = create_model_from_pretrained(model_cfg, checkpoint_path)
    return model

class PromptLearner(nn.Module):
    def __init__(self, classnames=["Negative", "Positive"], conch_model=None):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = 16
        ctx_init = ""
        # conch_model = load_conch_to_cpu()
        assert conch_model is not None
        dtype = torch.float32
        ...
        name_lens = [len(_tokenizer.encode(name)) for name in classnames]
        prompts = [name for name in classnames]
        tokenized_prompts = torch.cat([tokenize(_tokenizer, [p]) for p in prompts])
        with torch.no_grad():
...
class ViLa_MIL_Model(nn.Module):
    ...
    def forward(self, x_s, x_l, label):
        ...
        # return logits, Y_prob, Y_hat, None, loss
