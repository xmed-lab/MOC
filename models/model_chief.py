...
class CHIEF(nn.Module):
    def __init__(self, gate=True, size_arg="large", dropout=True, n_classes=2,
                 instance_loss_fn=nn.CrossEntropyLoss(), anatomic=None, **kwargs):
        ...
        if self.anatomic is None:
            self.anatomic = 0
        self.anatomic = torch.tensor([self.anatomic], dtype=torch.long)
        self.register_buffer('organ_embedding', torch.randn(19, 768))
        word_embedding = torch.load('/home/txiang/pathology/CLAM/models/CHIEF_text_emdding.pth')
    ...
    def forward(self, h, x_anatomic=None):
        if x_anatomic is None:
            x_anatomic = self.anatomic
        batch = x_anatomic
        h_ori = h
        ...
        embed_batch = embed_batch.to(WSI_feature.device)
        ...
        # return result
        return logits, Y_prob, Y_hat, A_raw, result

    def forward_patch_level(self, h, x_anatomic=None):
        if x_anatomic is None:
            x_anatomic = self.anatomic
        batch = x_anatomic
        A, h = self.attention_net(h)
        ...
        embed_batch = self.organ_embedding[batch]
        embed_batch=self.text_to_vision(embed_batch)
        embed_batch = embed_batch.to(h.device)
        M = torch.mm(A, h)
        M = M+embed_batch
    ...
    def patch_probs(self, h,x_anatomic=None):
        if x_anatomic is None:
            x_anatomic = self.anatomic
        batch = x_anatomic
        A, h = self.attention_net(h)
