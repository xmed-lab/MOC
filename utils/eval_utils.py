...
def eval_lynature(dataset, args, ckpt_path, patch_eval=False):
    # just store logits
    model = initiate_model(args, ckpt_path)
    loader = get_simple_loader(dataset)
    ...
    for batch_idx, (data, label) in tqdm(enumerate(loader)):
        ...
        if patch_eval:
            if all_logits is None:
                ...
                all_logits = np.concatenate((all_logits, logits.cpu().numpy()), axis=0)
        else:
            all_logits[batch_idx] = logits.cpu().numpy()
